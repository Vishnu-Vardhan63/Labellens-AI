...
import { validateScan, getScanEvidence, downloadReport } from '../api/client';
...
export default function ResultsPage() {
  ...
  const scanId = state?.scanId || analysis?.scan_id;
  const preview = state?.preview;

  // Fetch compliance validation and evidence
  useEffect(() => {
    if (!scanId) return;
    ...
    const loadData = async () => {
      try {
        const [vJson, eJson] = await Promise.all([
          validateScan(scanId),
          getScanEvidence(scanId),
        ]);

        if (isMounted) {
          if (vJson) {
            setValidationData(vJson);
          }
          if (eJson) {
        ...
        console.error('Failed to load validation or evidence:', err);
      } finally {
        if (isMounted) setIsLoadingValidation(false);
      }
    };
  ...
  const fields: ExtractedFields = analysis.extracted_fields || ({} as ExtractedFields);
  ...
  const isEligible =
    ...
    analysis?.label_detection?.is_eligible !== false;

  const handleCopyOcr = () => {
    if (analysis.raw_ocr_text) {
      navigator.clipboard.writeText(analysis.raw_ocr_text);
      setCopied(true);
  ...
  // ---------------------------------------------------------------------------
  if (!isLoadingValidation && !isEligible) {
    const isNoText =
      validationData?.overall_assessment === 'NO_READABLE_TEXT' ||
      analysis?.label_detection?.status === 'NO_READABLE_TEXT';
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 font-mono">
                  {analysis.ocr_summary?.line_count ?? analysis.ocr_lines?.length ?? 0} lines
                </span>
              </div>
              {showOcrText ? (
            ...
            {showOcrText && (
              <div className="p-4 border-t border-[#E5EAF0] bg-slate-50">
                <div className="flex items-center justify-between mb-3 text-xs text-text-secondary">
                  <span>
                    Engine: <strong className="text-text-primary">{analysis.ocr_summary?.engine || 'PaddleOCR (ONNX)'}</strong>
                  </span>
                  <button
                    type="button"
                ...
                <pre className="p-3 bg-white border border-[#E5EAF0] rounded-lg text-xs font-mono text-slate-800 whitespace-pre-wrap break-words max-h-60 overflow-y-auto leading-relaxed">
                  {analysis.raw_ocr_text || 'No readable text was detected.'}
                </pre>
              </div>
            )}
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#F6F8FB] pb-24 sm:pb-12">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          {/* RIGHT COLUMN: Summary & Declarations Accordion */}
            ...
            <div className="border border-[#E5EAF0] rounded-xl overflow-hidden bg-white mt-6">
              ...
              {showOcrText && (
                <div className="p-4 border-t border-[#E5EAF0] bg-slate-50">
                  <div className="flex items-center justify-between mb-3 text-xs text-text-secondary">
                    <span>
                      Engine: <strong className="text-text-primary">{analysis.ocr_summary?.engine || 'PaddleOCR (ONNX)'}</strong> · Avg Conf: <strong className="text-text-primary">{((analysis.ocr_summary?.average_confidence || 0) * 100).toFixed(1)}%</strong>
                    </span>
                    <button
                      type="button"
                      onClick={handleCopyOcr}
                  ...
                  <pre className="p-3 bg-white border border-[#E5EAF0] rounded-lg text-xs font-mono text-slate-800 whitespace-pre-wrap break-words max-h-60 overflow-y-auto leading-relaxed">
                    {analysis.raw_ocr_text || 'No text detected.'}
                  </pre>
                </div>
              )}
            </div>
